...
function buildAvatarUrls(category: PresetStyle, count = 6): string[] {
  return Array.from(
    { length: count },
    (_, i) => `/avatars/${category}/avatar-${i + 1}.svg`,
  );
}

const cartoonAvatars = buildAvatarUrls("cartoon");
const fantasyAvatars = buildAvatarUrls("fantasy");
const professionalAvatars = buildAvatarUrls("professional");
const minimalistAvatars = buildAvatarUrls("minimalist");
const animalsAvatars = buildAvatarUrls("animals");
const abstractAvatars = buildAvatarUrls("abstract");
...
export const allPresets: AvatarPreset[] = Object.values(
  presetCategories,
