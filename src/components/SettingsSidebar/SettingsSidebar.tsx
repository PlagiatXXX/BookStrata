...
const dangerActionButtonClass = `${secondaryActionButtonClass} border-(--theme-accent-secondary) text-(--theme-accent-secondary) hover:bg-(--theme-accent-secondary)/10`;
...
export const SettingsSidebar = memo(({
  ...
  return (
    <aside className="nb-sidebar relative flex w-full flex-col text-(--theme-text) lg:w-80">
      {onFindBook && (
        <div className="nb-section-header">
          <h3 className="nb-label-md mb-4 text-(--theme-accent-primary)">Поиск в библиотеке / LiveLib</h3>
          <button
            onClick={onFindBook}
      ...
      <div className="nb-section-header">
        <h3 className="nb-label-md mb-4 text-(--theme-accent-primary)">Управление блоками</h3>
      ...
      {activeTier && (
        <div className="nb-section-header nb-sidebar-active-tier">
          <h3 className="nb-label-md mb-4 text-(--theme-accent-tertiary)">Настройки: {activeTier.title}</h3>
          <div className="flex flex-col gap-6">
            <div className="flex items-center justify-between">
              <span className="nb-label-md">Высота строки</span>
              <div className="flex items-center gap-2 nb-heavy-border bg-(--theme-surface-4) p-1">
                <button
                  type="button"
                  onClick={() => handleHeightChange("decrease")}
                  aria-label="Уменьшить высоту строки"
                  className="flex size-8 items-center justify-center bg-(--theme-surface-4) text-(--theme-text) hover:bg-(--theme-accent-primary) hover:text-(--theme-on-accent) transition-colors focus-visible:ring-2 focus-visible:ring-(--theme-focus) focus:outline-none"
                >
                  <Minus size={14} />
                </button>
            ...
            {/* Начертание */}
            <div>
              <span className="nb-label-md mb-2 flex items-center gap-1.5 text-[10px] text-(--theme-text-muted) uppercase tracking-widest">
                  ...
                  return (
                    <button
                      ...
                      className={`flex-1 cursor-pointer nb-heavy-border px-1.5 py-1.5 text-[10px] font-black uppercase tracking-widest transition-all ${
                        isActive
                          ? "bg-(--theme-accent-primary) text-(--theme-on-accent) shadow-(--theme-shadow-sm)"
                          : "bg-(--theme-surface-4) text-(--theme-text) hover:bg-(--theme-surface-2)"
                      }`}
                    >
              ...
              <button
                ...
                className={`cursor-pointer nb-heavy-border px-3 py-1 text-[11px] font-black uppercase tracking-wider transition-all ${
                  activeTier?.labelStyle === "italic"
                    ? "bg-(--theme-accent-secondary) text-(--theme-on-accent) shadow-(--theme-shadow-sm)"
                    : "bg-(--theme-surface-4) text-(--theme-text) hover:bg-(--theme-surface-2)"
                }`}
              >
                  ...
                  return (
                    <button
                      ...
                      className={`nb-heavy-border size-6 cursor-pointer border transition-all ${
                        isActive
                          ? "scale-110 ring-2 ring-(--theme-accent-primary) ring-offset-1 ring-offset-(--theme-border)"
                          : "hover:scale-110"
                      }`}
                      style={{
      ...
      <div className="mt-auto flex flex-col gap-4 border-t border-(--theme-accent-primary)/10 pt-4">
        {onTogglePublic && (
          <div className="flex items-center justify-between nb-heavy-border bg-(--theme-surface-4) p-4">
            <label
              htmlFor="public-access-switch"
              className="flex cursor-pointer items-center gap-2"
            >
              <Globe size={16} className="text-(--theme-accent-primary)" />
        ...
        {shareUrl && (
          <div className="relative" ref={shareRef}>
            ...
            {showSharePopover && (
              <div className="absolute bottom-full left-0 right-0 mb-2 z-50 nb-heavy-border bg-(--theme-bg) p-3 shadow-(--theme-shadow)">
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <button
                    ...
                    title="Telegram"
                  >
                    <svg viewBox="0 0 24 24" fill="currentColor" className="size-7 text-(--theme-accent-primary)">
                      <path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0Zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635Z"/>
                    </svg>
                    <span className="text-xs uppercase font-bold text-(--theme-text-muted)">Telegram</span>
                  </button>
                  ...
                  >
                    ...
                    <span className="text-xs uppercase font-bold text-(--theme-text-muted)">VK</span>
                  </button>
                </div>
                ...
                <button
                  ...
                  className="mt-2 flex w-full items-center justify-center gap-2 border-2 border-(--theme-accent-primary)/20 px-3 py-2 text-[11px] font-black uppercase tracking-wider text-(--theme-text-muted) transition-colors hover:bg-white/5"
                >
                  <Copy size={14} className="text-(--theme-accent-primary)" />
