...
interface CuratorApplyModalProps {
  onClose: () => void
  onSuccess: () => void
}

export function CuratorApplyModal({ onClose, onSuccess }: CuratorApplyModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    setError(null)
    try {
      await applyGeneral(selectedId, message || undefined)
      setSuccess(true)
      onSuccess()
      sileo.success({ title: "Заявка отправлена", description: "Ваша заявка передана модераторам", duration: 4000 })
    } catch (err) {
        ...
        {success ? (
          <div className="text-center py-8">
            ...
            <p className="text-(--ink-1) text-sm mb-6">
              Ваша заявка передана модераторам. Когда появится подходящая битва, вас добавят.
            </p>
            <button
              onClick={onClose}
          ...
          <>
            <h3 className="text-xl font-black mb-2">Подать заявку на участие</h3>
            <p className="text-(--ink-1) text-sm mb-6">
              Выберите публичный тир-лист, который хотите выставить на будущие битвы. Модератор рассмотрит заявку.
            </p>
              ...
              <>
                ...
                <div className="mb-6">
                  ...
                  <textarea
                    ...
                    maxLength={500}
                    rows={3}
                    placeholder="Почему ваш тир-лист достоин участия?"
