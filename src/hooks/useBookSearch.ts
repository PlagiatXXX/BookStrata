...
function getCache(): Record<string, CachedResult> {
  return (
    StorageService.getJson<Record<string, CachedResult>>(SEARCH_CACHE_KEY) ?? {}
  );
}
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      ...
      if (uniqueBooks.length === 0) {
        ...
        // Добавляем только уникальные книги
        setResults((prev) => {
          const existingKeys = new Set(prev.map((b) => b.openLibraryKey));
