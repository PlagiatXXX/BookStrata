...
## 2026-05-06 - TypeScript Fixes in Achievements and Likes **Learning:** When adding new return fields like `newAchievements` to existing routes, ensure they are defined in all HTTP handlers (GET, POST, DELETE) to avoid runtime and compile-time errors. **Action:** Always verify all route handlers in a file when modifying the expected response structure.
