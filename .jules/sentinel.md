...
## 2026-04-19 - BOLA in Bulk Operations (saveTiers/saveAll)
**Vulnerability:** Broken Object Level Authorization (BOLA) in bulk save operations allowed users to update tiers and place books belonging to other users' tier lists.
**Learning:** Atomic bulk operations (like `saveAll` or `saveTiers` using `update`) often overlook individual resource ownership if they rely solely on primary keys. Even if an operation is wrapped in a transaction, it must explicitly scope every update and creation to the authorized parent resource.
**Prevention:** Use `updateMany` instead of `update` when the parent ID (e.g., `tierListId`) is available, adding it to the `where` clause. For operations involving related entities (like `BookPlacement`), perform a preliminary `count` or `findMany` check to ensure all provided IDs belong to the authorized context before proceeding with the write.

## 2026-04-22 - BOLA in Related Entity Associations (tierId)
**Vulnerability:** Broken Object Level Authorization (BOLA) in placement operations (`updatePlacements`, `saveAll`) allowed users to associate books with tiers belonging to other users' tier lists.
**Learning:** Scoping the primary entity (e.g., `tierListId`) is insufficient if the operation also links to other related entities (e.g., `tierId`). Without explicit ownership validation for the related IDs, an attacker can inject references to resources they don't own into their own records, potentially causing data corruption or leaking information about the structure of other users' data.
**Prevention:** Always validate that all referenced foreign keys (like `tierId` in a placement) belong to the authorized parent context (like `tierListId`) before performing upserts or bulk creations. Perform a preliminary `count` or `findMany` check on the related table, filtered by both the providing IDs and the parent ID.
