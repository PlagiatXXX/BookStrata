...
export async function saveTiers(
  ...
  const createdTiers = results.filter((t: any) => !updatedIds.has(t.id));
  const updatedTierList = results.filter((t: any) => updatedIds.has(t.id));
  ...
  return await prisma.$transaction(async (tx) => {
    ...
    if (payload.tiers) {
      ...
      if (payload.tiers.added?.length) {
        const addedTiers = await Promise.all(
        ...
        payload.tiers.added.forEach((tier, index) => {
          tierReplacementsMap.set(tier.tempId, String(addedTiers[index].id));
        });
      }
    }
    ...
    if (payload.newBooks?.length) {
      ...
      payload.newBooks.forEach((bookData, index) => {
        bookReplacementsMap.set(bookData.tempId, String(createdBooks[index].id));
      });
    }
