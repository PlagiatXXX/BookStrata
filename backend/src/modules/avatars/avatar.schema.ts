...
// Схема для загрузки аватара - предотвращает DoS-атаки через огромные полезные нагрузки (лимит 5МБ для base64 данных).
export const uploadAvatarBodySchema = z.object({
  avatar: z
    .string()
    .min(1, "Аватар обязателен")
    .max(5 * 1024 * 1024, "Размер аватара превышает лимит 5МБ"),
});
